/*  Program to further demonstrate fseek() and fread().
...
main()
{
  ...
  void display_supplier( int supp_code, FILE *fp ) ;
  FILE *fp ;
  int in_code ;
...
/*  Function : display_supplier()
    ...
    Arguments: Supplier code and file pointer.
    Returns  : void.                                         */
void display_supplier( const int supp_code, FILE *fp )
{
  struct supplier_rec
  {
    int code ;
